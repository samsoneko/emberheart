...
func update_minimap():
	...
	for x in range(dungeon_grid.size()):
		for y in range(dungeon_grid[0].size()):
			if dungeon_grid[x][y] != 0 && dungeon_grid[x][y] != 2 && dungeon_grid[x][y] != 8: # If grid is neither terrain nor fluid
				layers["terrain"].set_cell(Vector2i(x, y), 0, tile_dictionary["ground"])

func update_minimap_entities():
	layers["entities"].clear()
	var player_position = Global.player.get_player_grid_position()
	layers["entities"].set_cell(player_position, 0, tile_dictionary["player"])
	
	if Global.game_state == Global.GameStates.DUNGEON:
		var items = Global.current_scene.dungeon_items
		for item in items:
			layers["entities"].set_cell(item.grid_position, 0, tile_dictionary["item"])
